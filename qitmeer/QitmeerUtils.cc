...
static bool checkRPCNodeInfo(const string &rpcAddr, const string &rpcUserpass) {
  string response;
  string request = "{\"jsonrpc\":\"2.0\",\"id\":\"1\",\"method\":\"getNodeInfo\",\"params\":[]}";
  ...
  LOG(INFO) << "getNodeInfo: " << response;

  JsonNode r;
  if (!JsonNode::parse(
          response.c_str(), response.c_str() + response.length(), r)) {
    LOG(ERROR) << "decode getNodeInfo failure";
    return false;
  }
  ...
  if (r["result"].type() != Utilities::JS::type::Obj ||
    ...
    LOG(ERROR) << "getNodeInfo missing some fields";
    return false;
  }
...
bool checkRPC(const string &rpcAddr, const string &rpcUserpass) {
  return checkRPCNodeInfo(rpcAddr, rpcUserpass);
}
