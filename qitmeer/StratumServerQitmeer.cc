...
unique_ptr<StratumSession> ServerBitcoin::createConnection(
  ...
  return std::make_unique<StratumSessionQitmeer>(*this, bev, saddr, sessionID);
}
  ...
  dispatchToShareWorker([this,
    ...
    if (sjob->proxyJobDifficulty_ == 0 &&
      ...
      LOG(INFO) << "high diff share, blkhash: " << blkHash.ToString()
                << ", diff: " << QitmeerDifficulty::TargetToDiff(blkHash)
                << ", networkDiff: "
                << QitmeerDifficulty::TargetToDiff(sjob->networkTarget_)
                << ", by: " << workFullName;
    }
