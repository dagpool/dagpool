...
#include "StratumSessionQitmeer.h"
...
StratumSessionQitmeer::StratumSessionQitmeer(
...
uint16_t
StratumSessionQitmeer::decodeSessionId(const std::string &exMessage) const {
...
void StratumSessionQitmeer::sendSetDifficulty(
  ...
  uint256 target;
  QitmeerDifficulty::DiffToTarget(difficulty, target);
...
void StratumSessionQitmeer::sendMiningNotify(
...
void StratumSessionQitmeer::handleRequest(
...
void StratumSessionQitmeer::handleRequest_AgentGetCapabilities(
...
void StratumSessionQitmeer::handleRequest_MiningConfigure(
    const string &idStr, const JsonNode &jparams) {
...
void StratumSessionQitmeer::handleRequest_Subscribe(
    const string &idStr, const JsonNode &jparams) {
  if (state_ != CONNECTED) {
...
void StratumSessionQitmeer::handleRequest_Authorize(
    const string &idStr, const JsonNode &jparams) {
...
void StratumSessionQitmeer::handleRequest_SuggestTarget(
    const string &idStr, const JsonNode &jparams) {
  if (state_ != CONNECTED) {
  ...
  suggestedDiff_ = formatDifficulty(
      QitmeerDifficulty::TargetToDiff(jparams.children()->at(0).str()));
  responseTrue(idStr);
}

void StratumSessionQitmeer::logAuthorizeResult(
...
unique_ptr<StratumMessageDispatcher> StratumSessionQitmeer::createDispatcher() {
  if (isAgentClient_) {
...
uint8_t StratumSessionQitmeer::allocShortJobId() {
  // return range: [0, 9]
...
unique_ptr<StratumMiner> StratumSessionQitmeer::createMiner(
...
void StratumSessionQitmeer::responseError(const string &idStr, int errCode) {
